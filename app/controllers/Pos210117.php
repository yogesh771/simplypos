...
$print = array();
		...
		foreach($print['rows'] as $key => $row){
			foreach($row as $key2 => $value){
				if($key2 == 'quantity'){
					$print['rows'][$key]->$key2 = round($value);
				}
		...
		foreach($print['inv'] as $key => $row){
			$print['inv']->$key = round($row);
		}
		//echo '<pre>';
		//var_dump($print['rows'][0]);
		//echo '<br>';
		//var_dump($print['qrcode']);
		//exit;
		if($sale_id != $_SESSION['print']){
			?>
			<script>
	...
	function testing(){
		...
		$logger = array('Testing error 123' , $errorUrl);
		$this->pos_error_log($logger);
	}
}