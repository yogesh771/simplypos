                                            ...
                                            <table id="purchases-tbl" cellpadding="0" cellspacing="0" border="0"
												...
												if (!empty($apinotify)) {
                                                    ...
                                                    foreach ($apinotify as $apinotifys) {
														...
														<td id="customer_code" calss="hide" style="display: none;">' . $apinotifys->customer_code . '</td>
														<td id="customer_name">' . $apinotifys->customer_name . '</td>
														<td>' . $apinotifys->comment . '</td>
                                                   <td><center><button onClick="accept('.$apinotifys->merchant_code.','.$apinotifys->customer_code.',\''.$apinotifys->customer_name.'\')" class="accept label label-success">Accept</button>&nbsp;&nbsp;<button onClick="decline('.$apinotifys->merchant_code.','.$apinotifys->customer_code.',\''.$apinotifys->customer_name.'\')" class="decline label label-danger">Decline</button></center></td>
                                                </tr>';
                                                        $r++;
                                                    }
                                                } else { ?>
    ...
    $(document).ready(function () {
	 ...
	 //accept//
		...
		function accept(merchant_code,customer_code,customer_name){
			//alert(merchant_code);
			//alert(customer_name);
			var data = "action=updateStatus&merchant="+merchant_code+"&customerMobile="+customer_code+"&status=accepted";
								$.ajax({
								type: "get",
								data: data,
		...
		function decline(merchant_code,customer_code,customer_name){
			//alert(merchant_code);
			//alert(customer_code);
			var data = "action=updateStatus&merchant="+merchant_code+"&customerMobile="+customer_code+"&status=declined";
								$.ajax({
								type: "get",
								data: data,
								url: "https://simplypos.in/api/merchant-api.php",
